...
public:
    ...
    bool search(vector<vector<char>>& board, vector<vector<bool>>& visited, int i, int j, int idx, const string &word) {
        ...
        visited[i][j] = false;  // Backtrack after checking all directions
        return false;
    }
...
int main() {
    ...
    string word = "ABCCED";
    cout << (sol.exists(board, word) ? "True" : "False") << endl; // Expected Output: True

    word = "SEE";
    cout << (sol.exists(board, word) ? "True" : "False") << endl; // Expected Output: True

    word = "ABCB";
    cout << (sol.exists(board, word) ? "True" : "False") << endl; // Expected Output: False
